...
def main():
    ...
    model = green_ampt().green_ampt_model(df, P_value)
    model.to_csv('results/results_green_ampt_manual_2.csv', sep=';', index=False)
    plot_results(model)
...
class green_ampt():
    ...
    def green_ampt_model(self,df, P_value):
        time_list = []
        time_list_date = []
        ...
        for index, value in enumerate(time_list):
            if index == 0:
                ...
                esuperficial_list.append(rsuperficial_list[index]*(1-config['key_value']**(1/config['Ksup'])))      
                rsolo_list.append(infiltration_list[index] + config['initial_soil_water'])
                esubterranea_list.append(rsolo_list[index]*(1-config['key_value']**(1/config['Ksub'])))
                rec_list.append(rsolo_list[index]*(1-config['key_value']**(1/config['Kperc'])))
                rsubterranea_list.append(rec_list[index] + config['initial_rsub_water'])
                ebase_list.append(rsubterranea_list[index]*(1-config['key_value']**(1/config['Kbase'])))
